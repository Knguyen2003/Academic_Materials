        ...
        public  static List<ATMMoneyItem> GetMoneyStore()
        {
            ...
            if (mn.Rows.Count > 0)
            {
                ...
                while (count < mn.Rows.Count)
                {
                    ...
                    {
                        Denomination = Convert.ToSingle(mn.Rows[count]["Denomination"].ToString()),
                        Currency = mn.Rows[count]["Currency"].ToString(),
                        Quantity = Convert.ToInt32(mn.Rows[count]["Quantity"].ToString())
                    };
                    list.Add(money);
                    count++;
                }
